...
if(isset($_GET['id'])){
    $decrypted_id_terrain = openssl_decrypt($_GET['id'], "AES-128-ECB", "uiop5258");
    try {
        $requete = $bdd->prepare("SELECT * FROM terrain WHERE id_terrain = ?");
        $requete->execute(array($decrypted_id_terrain));
...
$events="";
try {
    $requete_plage_horaire = $bdd->prepare("SELECT id_plage_horaire FROM horaires WHERE id_terrain = :terrain_id");
    $requete_plage_horaire->bindParam(':terrain_id', $terrain['id_terrain']);
    $requete_plage_horaire->execute();

    while ($plage_horaire = $requete_plage_horaire->fetch()) {
        $plage_horaire_id = $plage_horaire['id_plage_horaire'];

        $requete_reservations = $bdd->prepare("
            SELECT reservations.statut, horaires.*
            FROM reservations 
            INNER JOIN horaires ON reservations.id_plage_horaire = horaires.id_plage_horaire
            WHERE reservations.id_plage_horaire = :plage_horaire_id
        ");
        $requete_reservations->bindParam(':plage_horaire_id', $plage_horaire_id);
        $requete_reservations->execute();

        while ($reservation = $requete_reservations->fetch()) {
            $events .= "{
                title: 'Réservé',
                start: '" . $reservation['jour_semaine'] . "T" . $reservation['heure_debut'] . "',
                end: '" . $reservation['jour_semaine'] . "T" . $reservation['heure_fin'] . "',
                backgroundColor: '" . ($reservation['statut'] === 'En attente' ? '#fccb05' : '#00ff00') . "',
                borderColor: 'transparent'
            },";
        }
        $requete_reservations->closeCursor();
    }
    $requete_plage_horaire->closeCursor();
} catch (PDOException $e) {
...
<head>
  ...
  <title>Kram foot | terrain : <?php echo  $terrain['nom_terrain']  ?></title>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            headerToolbar: {
                left: 'prev,next',
                center: 'title',
                right: 'dayGridMonth,timeGridWeek,dayGridWeek,dayGridDay'
            },
            events: [
              <?php echo $events?>
  ...
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
    ...
    <div class="body-wrapper">
              ...
              <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-animate-up" aria-labelledby="drop2">
                  <div class="message-body">
                    ...
                    <a class="btn btn-outline-success mx-3 mt-2 d-block" href="../index.html">Logout</a>
                  </div>
                </div>
              </li>
            </ul>
      ...
      <div class="container-fluid">
        <div class="row">
            <div class="col">
                <h1><?php echo $terrain['nom_terrain'] ?></h1>
                <p><?php echo $terrain['description'] ?></p>
                <p>Prix : <?php echo $terrain['prix'] ?> DNT</p>
                <?php
                    echo "<img src='data:image/jpeg;base64," . base64_encode($terrain['image']) . "' alt='' class='rounded img-fluid'>";
            ...
            <form method="post">
                <div class="mb-3">
                    ...
                    <input type="date" name="jour_semaine" id="jour_semaine" class="form-control" required>
...
if (isset($_POST['jour_semaine']) && isset($_POST['heure_debut']) && isset($_POST['heure_fin'])){
    $jour_semaine = $_POST['jour_semaine'];
    $heure_debut = $_POST['heure_debut'];
    $heure_fin = $_POST['heure_fin'];

    try {
        $stmt_verification = $bdd->prepare("SELECT COUNT(*) AS nb_reservations FROM reservations AS r 
            INNER JOIN horaires AS p ON r.id_plage_horaire = p.id_plage_horaire 
            WHERE p.jour_semaine = :jour_semaine 
            AND (
                (p.heure_debut <= :heure_debut AND p.heure_fin >= :heure_debut) OR 
                (p.heure_debut <= :heure_fin AND p.heure_fin >= :heure_fin) OR 
                (p.heure_debut >= :heure_debut AND p.heure_fin <= :heure_fin)
            )
            and
            p.id_terrain = :id_terrain
            ");
        $stmt_verification->bindParam(':jour_semaine', $jour_semaine);
        $stmt_verification->bindParam(':heure_debut', $heure_debut);
        $stmt_verification->bindParam(':heure_fin', $heure_fin);
        $stmt_verification->bindParam(':id_terrain', $terrain['id_terrain']);
        $stmt_verification->execute();
        $result_verification = $stmt_verification->fetch(PDO::FETCH_ASSOC);

        if ($result_verification['nb_reservations'] > 0) {
            echo "<div class='alert alert-danger' role='alert'>Ce terrain est déjà réservé pendant cette période.</div>";
        } else {
            try {
                $stmt_plage_horaire = $bdd->prepare("INSERT INTO horaires (jour_semaine,id_terrain, heure_debut, heure_fin) VALUES (:jour_semaine,:id_terrain, :heure_debut, :heure_fin)");
               
                $stmt_plage_horaire->bindParam(':jour_semaine', $jour_semaine);
                $stmt_plage_horaire->bindParam(':heure_debut', $heure_debut);
                $stmt_plage_horaire->bindParam(':heure_fin', $heure_fin);
                $stmt_plage_horaire->bindParam(':id_terrain', $terrain['id_terrain']);
                $stmt_plage_horaire->execute();
        
                $id_plage_horaire = $bdd->lastInsertId();
        
               
                $statut = "Valide";
        
                $stmt_reservation = $bdd->prepare("INSERT INTO reservations (id_plage_horaire,id_user, statut) VALUES (:id_plage_horaire,:id_user, :statut)");
                $stmt_reservation->bindParam(':id_plage_horaire', $id_plage_horaire);
                $stmt_reservation->bindParam(':id_user', $user['id_user']);
                $stmt_reservation->bindParam(':statut', $statut);
                $stmt_reservation->execute();
        
                echo "<div class='alert alert-success' role='alert'>Réservation effectuée avec succès!</div>";
            } catch (PDOException $e) {
                echo "<div class='alert alert-danger' role='alert'>Erreur lors de la réservation : " . $e->getMessage() . "</div>";
            }
        }
    } catch (PDOException $e) {
        echo "<div class='alert alert-danger' role='alert'>Erreur lors de la vérification de la réservation : " . $e->getMessage() . "</div>";
    }
} else {
...
?>
        ...
        <div class="row">
            <div id="calendar" style="height:100%; margin-top:5%"></div>
        ...
        <div class="py-6 px-6 text-center">
          <p class="mb-0 fs-4">Design and Developed by <a href="https://jerbi-portfolio.netlify.app/" target="_blank" class="pe-1 text-success text-decoration-underline">Jerbi Ahmed</a></p>
        </div>
      </div>
    </div>
  </div>
