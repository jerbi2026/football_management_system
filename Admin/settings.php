...
<head>
  ...
  <title>Kram foot | settings : <?php echo  $user['username']  ?></title>
...
<body>
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
    ...
    <div class="body-wrapper">
      ...
      <header class="app-header">
       ...
       clas            <p class="mb-0 fs-3">My Profile</p>
                    </a>
                    <a href="javascript:void(0)" class="d-flex align-items-center gap-2 dropdown-item">
                      <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-report"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h5.697" /><path d="M18 14v4h4" /><path d="M18 11v-4a2 2 0 0 0 -2 -2h-2" /><path d="M8 3m0 2a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2v0a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2z" /><path d="M18 18m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /><path d="M8 11h4" /><path d="M8 15h3" /></svg>
  ...
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact" type="button" role="tab" aria-controls="contact" aria-selected="false">Activty</button>
  </li>
</ul>
<div class="tab-content" id="myTabContent">
        ...
        <div class="col ">
            ...
            <div class="card overflow-hidden rounded-2">
              ...
              <div class="card-body pt-3 p-4">
                <h6 class="fw-semibold fs-4">username : <?php echo $user['username'] ?></h6>
                <div class="d-flex align-items-center justify-content-between">
                  <h6 class="fw-semibold fs-4 mb-0"><?php echo $user['email'] ?></h6><br>
                 <p>date creation : <?php echo $user['date_creation'] ?></p>
                </div>
              </div>
            </div>
          </div>
  ...
  <form method="post" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="pdp" class="form-label">Photo de profile</label>
                    <input type="file" class="form-control" id="pdp" name="pdp" required aria-describedby="pdphelp">
                    <div id="pdphelp" class="form-text">Veuillez choisir un logo dont la taille est inférieure à 64ko sinon elle ne va pas etre affichée correctement</div>
                </div>
                <div class="mb-3">
                    <label for="username" class="form-label">Username</label>
                    <input type="text" class="form-control" id="username" name="username" aria-describedby="usernamehelp" value=<?php echo $user['username'] ?> required>
                    <div id="usernamehelp" class="form-text">Veuillez choisir un nom pour votre équipe</div>
                </div>
                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="text" class="form-control" id="email" name="email" aria-describedby="emailhelp" value=<?php echo $user['email'] ?> disabled readonly>
                    <div id="emailhelp" class="form-text">Email inchangeable, contactez admin pour modifier le mail</div>
                </div>
                <div class="mb-3">
                    <label for="password" class="form-label">Password</label>
                    <input type="password" class="form-control" id="password" name="password" aria-describedby="passwordhelp"  >
                    <div id="usernamehelp" class="form-text">Veuillez choisir un mot de passe fort (8 à 32 caractères)</div>
                </div>
                <div class="mb-3">
                    <label for="repeat_pass" class="form-label">Repeat Password</label>
                    <input type="password" class="form-control" id="repeat_pass" name="repeat_pass"  >
                </div>
                
    
                <button type="submit" name="submit" class="btn btn-success m-1">Modifier</button>
    </form>
    <?php
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $pdp_data = file_get_contents($_FILES['pdp']['tmp_name']);
    ...
    if (!empty($password)) {
        if ($password === $repeat_pass) {
            ...
            try {
                $stmt = $bdd->prepare("UPDATE user SET username = :username, image = :pdp, password = :password WHERE email = :email");
                $stmt->bindParam(':username', $username);
                $stmt->bindParam(':pdp', $pdp_data, PDO::PARAM_LOB);
                $stmt->bindParam(':password', $hashed_password);
                $stmt->bindParam(':email', $user['email']);
                $stmt->execute();
                echo "<div class='alert alert-success' role='alert'>Les informations de l'utilisateur ont été mises à jour avec succès.</div>";
                
            } catch(PDOException $e) {
                echo "<div class='alert alert-danger' role='alert'>Erreur lors de la mise à jour des informations de l'utilisateur : " . $e->getMessage() . "</div>";
            }
        } else {
        ...
        try {
            $stmt = $bdd->prepare("UPDATE user SET username = :username, image = :pdp WHERE email = :email");
            $stmt->bindParam(':username', $username);
            $stmt->bindParam(':pdp', $pdp_data, PDO::PARAM_LOB);
            $stmt->bindParam(':email', $user['email']);
            $stmt->execute();
