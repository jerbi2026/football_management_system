...
$stmt->execute();
$commandes_count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

$query = "SELECT COUNT(*) as count FROM produit";
$stmt = $bdd->prepare($query);
$stmt->execute();
$produits_count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Récupérer le nombre de terrains
$query = "SELECT COUNT(*) as count FROM terrain";
$stmt = $bdd->prepare($query);
$stmt->execute();
$terrains_count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

// Récupérer le nombre de tournois
$query = "SELECT COUNT(*) as count FROM tournois";
$stmt = $bdd->prepare($query);
$stmt->execute();
$tournois_count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
?>

<?php 
$query = "SELECT MONTH(horaires.jour_semaine) as month, terrain.nom_terrain, COUNT(*) as count
FROM horaires
JOIN terrain ON horaires.id_terrain = terrain.id_terrain
join reservations on horaires.id_plage_horaire = reservations.id_plage_horaire
GROUP BY terrain.nom_terrain, MONTH(horaires.jour_semaine)";
$stmt = $bdd->prepare($query);
$stmt->execute();
...
$terrain = $row['nom_terrain'];
if (!isset($data[$terrain])) {
$data[$terrain] = array_fill(0, 12, 0);
$terrains[] = $terrain;
}
$data[$terrain][$row['month'] - 1] = $row['count'];
}

$labels_json = json_encode($labels);
$data_json = json_encode($data);
$terrains_json = json_encode($terrains);
?>
?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: [
              <?php
                try {
                  $requete = $bdd->query("SELECT * FROM tournois");
                  while ($tournoi = $requete->fetch()) {
                  echo "{
                      title: '" . $tournoi['nom_tournoi'] . "',
                      start: '" . $tournoi['date_debut'] . "',
                      end: '" . $tournoi['date_fin'] . "',
...
<body>
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
        ...
        <div class="col-md-3">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-muted">Nombre de commandes</h6>
                    <p class="card-text"><?php echo $commandes_count; ?></p>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-muted">Nombre de produits</h6>
                    <p class="card-text"><?php echo $produits_count; ?></p>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-muted">Nombre de terrains</h6>
                    <p class="card-text"><?php echo $terrains_count; ?></p>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-muted">Nombre de tournois</h6>
                    <p class="card-text"><?php echo $tournois_count; ?></p>
                </div>
            </div>
        </div>
    </div>
   ...
   <div class="row">
      <h2>Statistiques des Réservations par Terrain</h2>
...
var terrains = <?php echo $terrains_json; ?>;

// Préparer les datasets pour Chart.js
var datasets = [];
var colors = ['rgba(54, 162, 235, 0.2)', 'rgba(75, 192, 192, 0.2)', 'rgba(255, 206, 86, 0.2)', 'rgba(153, 102, 255, 0.2)', 'rgba(255, 99, 132, 0.2)'];
var borderColors = ['rgba(54, 162, 235, 1)', 'rgba(75, 192, 192, 1)', 'rgba(255, 206, 86, 1)', 'rgba(153, 102, 255, 1)', 'rgba(255, 99, 132, 1)'];
