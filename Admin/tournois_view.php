...
<body>
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
    ...
    <div class="body-wrapper">
      ...
      <a class="btn btn-primary" data-bs-toggle="offcanvas" href="#offcanvasTournoi" role="button" aria-controls="offcanvasTournoi">
 Ajouter un tournois
</a>
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasTournoi" aria-labelledby="offcanvasTournoiLabel">
  <div class="offcanvas-header">
    <h5 class="offcanvas-title" id="offcanvasTournoiLabel">Ajouter un Tournoi</h5>
  ...
  <div class="offcanvas-body">
    ...
    <form id="tournoi-form" method="POST" action="crud_tournoi.php">
      <div class="mb-3">
      ...
      <div class="mb-3">
        <label for="id_terrain" class="form-label">Terrain</label>
        <select class="form-control" id="id_terrain" name="id_terrain" required>
          <!-- Options seront insérées ici depuis la base de données -->
          <?php
            include('php_fonctions/connexion.php');
            $query = "SELECT id_terrain, nom_terrain FROM terrain";
            $stmt = $bdd->prepare($query);
            $stmt->execute();
            $terrains = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($terrains as $terrain) {
              echo "<option value='" . $terrain['id_terrain'] . "'>" . $terrain['nom_terrain'] . "</option>";
            }
          ?>
        </select>
      </div>
      <div class="mb-3">
        <label for="nb_equipe" class="form-label">Nombre d'Équipes</label>
        <input type="number" class="form-control" id="nb_equipe" name="nb_equipe" required>
      </div>
      <div class="mb-3">
            ...
            while ($tournois = $requete->fetch()) {
              ...
              <div class='card-body'>
                ...
                if($tournois['nb_equipe_restant'] > 0) {
                  echo "<a  class='card-link' href='tournois.php?id=" . $encrypted_id_tournoi . "'>Voir tournois</a>";
                  echo "<a  class='card-link text-danger' href='tournois.php?id=" . $encrypted_id_tournoi . "'>Supprimer tournois</a>";
                } else {
