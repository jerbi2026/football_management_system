...
if(isset($_GET['id'])){
    $decrypted_produit = openssl_decrypt($_GET['id'], "AES-128-ECB", "uiop5258");
    try {
        $requete = $bdd->prepare("SELECT * FROM produit WHERE id_produit = ?");
        $requete->execute(array($decrypted_produit));
...
<head>
  ...
  <title>Kram foot | produit : <?php echo  $produit['nom_produit']  ?></title>
      ...
      <div class="container-fluid">
        <div class="row">
            <div class="col">
                ...
                <?php echo "<img src='data:image/jpeg;base64," . base64_encode($produit['image']) . "' alt='" . $produit['nom_produit'] . "' />"?> 
            ...
            <h5>Prix : <?php echo $produit['prix_produit'] ?> DNT</h5>
            <h5>Quantite : <?php echo $produit['quantite'] ?></h5>
            <?php
                echo"<a onclick='addToCart(\"" . $produit['id_produit'] . "\", \"" . $userId . "\")' class='btn btn-success'>Ajouter au panier</a>"; 
            ?>  
       ...
       <?php 
        echo"<div class='row'>";
        $requeteProduit = $bdd->prepare("SELECT * FROM produit WHERE id_categorie = ? and id_produit != ?");
        $requeteProduit->execute(array($produit['id_categorie'], $produit['id_produit']));
        
        while ($produit_autres = $requeteProduit->fetch()) {
          $encrypted_id_produit = openssl_encrypt($produit_autres['id_produit'], "AES-128-ECB", "uiop5258");
          
          echo "<div class='col-sm-6 col-xl-3'>
                  <div class='card overflow-hidden rounded-2'>
                    <div class='position-relative'>
                      <a href='javascript:void(0)'><img src='data:image/jpeg;base64," . base64_encode($produit_autres['image']) . "' alt='" . $produit_autres['nom_produit'] . "' /></a>
                      <a href='javascript:void(0)' class='bg-success rounded-circle p-2 text-white d-inline-flex position-absolute bottom-0 end-0 mb-n3 me-3' data-bs-toggle='tooltip' data-bs-placement='top' data-bs-title='Add To Cart' onclick='addToCart(\"" . $produit_autres['id_produit'] . "\", \"" . $userId . "\")'><i class='ti ti-basket fs-4'></i></a>
                      </div>
                    <div class='card-body pt-3 p-4'>
                      <h6 class='fw-semibold fs-4'>" . $produit_autres['nom_produit'] . "</h6>
                      <div class='d-flex align-items-center justify-content-between'>
                        <h6 class='fw-semibold fs-4 mb-0'>" . $produit_autres['prix_produit'] . "DNT </h6>
                      ...
                      <a href='produit_details.php?id=" . $encrypted_id_produit . "' class='btn btn-success'>View More</a>
                    </div>
                  </div>
                </div>";
        }
