          ...
          <div class="unlimited-access hide-menu bg-light-primary position-relative mb-7 mt-5 rounded">
            <div class="d-flex">
              <div class="unlimited-access-title me-3">
                ...
                <a class="btn btn-primary fs-2 fw-semibold lh-sm">Buy Pro</a>
              </div>
      ...
      <div class="container-fluid">
        <div class="row">
            <form method="post" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="titre" class="form-label">Titre de rapport</label>
                    <input type="text" class="form-control" id="titre" name="titre" required aria-describedby="titrehelp">
                    <div id="titrehelp" class="form-text">Maximum 30 caractères</div>
                </div>
                <div class="mb-3">
                    <label for="texte" class="form-label">Texte </label>
                    <textarea type="text" class="form-control" id="texte" name="texte" required></textarea>
                </div>
                <div class="mb-3">
                    <label for="file" class="form-label">File</label>
                    <input type="file" class="form-control" id="file" name="file"  aria-describedby="filehelp">
                    <div id="filehelp" class="form-text">Veuillez choisir un fichier dont la taille est inférieure à 64ko sinon elle ne va pas etre affichée correctement</div>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
        if ($_FILES['file']['size'] <= 64 * 1024) { 
            ...
            try {
                $stmt = $bdd->prepare("INSERT INTO report (titre, texte,file,id_user) VALUES (:titre, :texte, :fichier,:id_user)");

                $stmt->bindParam(':titre', $titre);
                $stmt->bindParam(':texte', $texte);
                $stmt->bindParam(':id_user', $user['id_user']);
                $stmt->bindParam(':fichier', $file_data, PDO::PARAM_LOB);

                if ($stmt->execute()) {
                    echo "<div class='alert alert-success' role='alert'>Le rapport a été inséré avec succès.</div>";
                } else {
                    echo "<div class='alert alert-danger' role='alert'>Erreur lors de l'insertion du rapport.</div>";
                }
            } catch(PDOException $e) {
                echo "<div class='alert alert-danger' role='alert'>Erreur lors de l'insertion du rapport : " . $e->getMessage() . "</div>";
            }
        } else {
            echo "<div class='alert alert-danger' role='alert'>La taille du fichier est supérieure à 64 Ko.</div>";
        }
    } else {
        ...
        try {
            $stmt = $bdd->prepare("INSERT INTO report (titre, texte,id_user) VALUES (:titre, :texte,:id_user)");
