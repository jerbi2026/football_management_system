...
if (isset($_POST['month_exp'])) {
    $current_user_id = $_SESSION['user_id'];
    $expiration_date = $_POST['month_exp'] . '-01';

    $query = "UPDATE type_user SET date_expiration = :date_expiration, type = :type WHERE id_type = :id_type";
    $stmt = $bdd->prepare($query);
    $type = 'premium';
    $stmt->bindParam(':date_expiration', $expiration_date);
    $stmt->bindParam(':type', $type);
    $stmt->bindParam(':id_type', $current_user_id);
          ...
          <div class="col-md-8 col-lg-6 col-xxl-3">
            <div class="card mb-0">
              <div class="card-body">
                ...
                <p class="text-center">Arena</p>
                <form id="upgrade-form" method="POST" action="upgrade_user.php">
                    <div class="mb-3">
                        <label for="month_exp" class="form-label">Expiration Date</label>
                        <input type="month" class="form-control" id="month_exp" name="month_exp" aria-describedby="textHelp" required>
                    </div>
                    <button type="submit" class="btn btn-primary w-100 py-8 fs-4 mb-4 rounded-2">Upgrade</button>
                </form>
              </div>
            </div>
          </div>
